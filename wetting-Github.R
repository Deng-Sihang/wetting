...
p.ds=if (ds.obs < 0) {valid <- ds.ran < ds.obs
  same_sign <- (sign(slop.ran[1, ]) == sign(slope.obs[1])) & 
    (sign(slop.ran[2, ]) == sign(slope.obs[2]))
  proportion <- sum(valid & same_sign) / length(ds.ran)} else {
    valid <- ds.ran > ds.obs
    same_sign <- (sign(slop.ran[1, ]) == sign(slope.obs[1])) & 
      (sign(slop.ran[2, ]) == sign(slope.obs[2]))
    proportion <- sum(valid & same_sign) / length(ds.ran)}
